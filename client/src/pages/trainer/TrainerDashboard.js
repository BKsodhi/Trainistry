...
function TrainerDashboard() {
  ...
  const fetchAll = async () => {
    try {
      ...
      // Parallelize fetching of stats, jobs, application states, and real-time alert documents
      const [dashRes, projRes, appRes, notifRes] = await Promise.all([
        axios.get("http://localhost:5000/api/trainer/dashboard", config),
        axios.get("http://localhost:5000/api/trainer/projects", config),
        axios.get("http://localhost:5000/api/trainer/applications", config),
  ...
  const handleMarkAsRead = async (notifId) => {
    try {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      await axios.put(`http://localhost:5000/api/notifications/${notifId}/read`, {}, config);
    ...
    <>
      <header className="header-section" style={{ position: "relative" }}>
        ...
        {data.myApplications?.some(app => ['interview_scheduled', 'selected', 'rejected'].includes(app.status)) && (
          <div className="payment-alert-box glass" style={{ marginBottom: '25px', padding: '20px', borderLeft: '5px solid #10b981' }}>
            <h3 style={{ margin: '0 0 12px 0', color: '#10b981' }}>🎯 Direct Pipeline Activity Updates</h3>
            {data.myApplications.map(app => {
              if (!['interview_scheduled', 'selected', 'rejected'].includes(app.status)) return null;
              return (
                <div key={app._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #f1f5f9' }}>
                  <div>
                    <strong style={{ fontSize: "15px" }}>{app.project?.title}</strong>
                    <p style={{ margin: "4px 0 0 0", fontSize: "13px", color: "var(--text-muted)" }}>Company: {app.project?.company?.name}</p>
                  </div>
                  
                  <div>
                    {app.status === 'interview_scheduled' && (
                      <div style={{ textAlign: "right" }}>
                        <span style={{ background: "#fffbeb", color: "#f59e0b", padding: "4px 12px", borderRadius: "100px", fontWeight: "bold", fontSize: "12px" }}>🗓️ Interview Set</span>
                        <div style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "4px" }}>
                          {app.interviewDate ? new Date(app.interviewDate).toLocaleDateString() : ""} at {app.interviewTime}
                        </div>
                        {app.meetingLink && (
                          <a href={app.meetingLink} target="_blank" rel="noreferrer" style={{ fontSize: "12px", color: "#4338ca", fontWeight: "bold", display: "block", marginTop: "2px" }}>Join Room</a>
                        )}
                      </div>
                    )}
                    {app.status === 'selected' && (
                      <span style={{ background: "#ecfdf5", color: "#10b981", padding: "6px 14px", borderRadius: "100px", fontWeight: "700", fontSize: "13px" }}>🎉 Hired & Selected</span>
                    )}
                    {app.status === 'rejected' && (
                      <span style={{ background: "#fef2f2", color: "#ef4444", padding: "6px 14px", borderRadius: "100px", fontWeight: "700", fontSize: "13px" }}>Passed Over</span>
                    )}
                  </div>
                </div>
              );
        ...
        {data.myApplications?.some(app => getDeadlineInfo(app)) && (
          <div className="payment-alert-box glass" style={{ marginBottom: '25px', padding: '20px', borderLeft: '5px solid #4338ca' }}>
            ...
            {data.myApplications.map(app => {
              ...
              if (!info) return null;
              return (
                <div key={app._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #eee', opacity: info.paymentStatus === 'cleared' ? 0.8 : 1 }}>
                  <div>
                    <strong>{app.project.title}</strong>
                    <div style={{ fontSize: '0.8rem', color: '#666' }}>
                        {info.paymentStatus === 'cleared' ? `Paid on: ${new Date().toLocaleDateString('en-IN')}` : `Deadline: ${info.date}`}
