...
function Network() {
    ...
    if (query.length > 2) {
      try {
        const res = await axios.get(
          `${API_BASE_URL}/api/trainer/search?name=${query}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const handleFollow = async (targetUserId) => {
    ...
    try {
      const res = await axios.put(
        `${API_BASE_URL}/api/trainer/follow/${targetUserId}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleLike = async (id) => {
    try {
      const res = await axios.put(
        `${API_BASE_URL}/api/achievements/${id}/like`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleCommentSubmit = async (id) => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE_URL}/api/achievements/${id}/comment`,
        { text: commentText },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleRepost = async (id) => {
    try {
      await axios.post(
        `${API_BASE_URL}/api/achievements/${id}/repost`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
          ...
          <>
                ...
                >
                  {searchResults.map((profile) => (
                    ...
                    >
                      ...
                      >
                        {followingList.includes(profile.user?._id)
                          ? "✓ Connected"
                          : "+ Connect"}
                      </button>
                    </div>
            ...
            <div className="post-trigger-box">
              ...
              <button onClick={() => setShowPostModal(true)}>
                Share a project completion or certificate...
              </button>
            </div>
