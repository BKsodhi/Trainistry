...
//   // HELPER: Opens the resume in a new tab securely
//   const handleViewResume = (e, resumePath) => {
//     e.preventDefault();
//     if (!resumePath) return alert("No resume found.");
...
//                 <div className="app-card-header">
//                   <div className="trainer-avatar">
//                     {/* OPTIONAL CHAINING: Prevents crash if user name is missing */}
//                     {app?.trainer?.user?.name?.charAt(0) || "T"}
//                   </div>
//                   <span className={`status-badge status-${app?.status?.toLowerCase() || "applied"}`}>
//                     {app?.status?.replace('_', ' ') || "Applied"}
...
//                 <div className="trainer-info">
//                   <h3>{app?.trainer?.user?.name || "Unnamed Trainer"}</h3>
//                   <p className="trainer-email">{app?.trainer?.user?.email || "No Email Provided"}</p>
//                 </div>
...
//                   <div className="stat">
//                     <span>Expected Rate</span>
//                     <strong>₹{app?.expectedRate || "0"} / day</strong>
//                   </div>
//                 </div>
...
//                       onClick={(e) => handleViewResume(e, app.resumeUrl)}
//                       className="resume-link-btn"
//                       style={{ border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left', color: '#2563eb', fontWeight: '500' }}
//                     >
//                       📄 View Resume
//                     </button>
//                   )}
...
//       {/* MODAL: Added safety checks for selectedApp */}
//       {showModal && (
//         <div className="modal-overlay">
//           <div className="modal-content glass">
//             <div className="form-header-section">
//               <h2 className="form-title">Schedule Interview</h2>
//               <p className="form-subtitle">with {selectedApp?.trainer?.user?.name || "Trainer"}</p>
//             </div>
//             <form onSubmit={handleScheduleInterview}>
//               <div className="form-group">
//                 <label>Date</label>
          ...
          {applications.length === 0 ? (
            ...
            applications.map((app) => (
              <div key={app?._id} className="application-card glass">
                ...
                {/* CONDITIONAL ADVANCE DISPLAY */}
                {(app?.status === "selected" || app?.status === "hired") && requiresAdvance(app) && (
                  <div className="advance-display-box">
                    <span style={{ fontSize: "0.85rem", color: "#666" }}>Advance (50%)</span>
                    <strong style={{ color: "#10b981", fontSize: "1.05rem" }}>
                      ₹
                      {(
                        (app?.expectedRate || app?.project?.perDayPayment || 0) *
                        (app?.project?.durationDays || 0) *
                        0.5
                      ).toLocaleString()}
                    </strong>
                  </div>
                ...
                <div className="app-footer-actions">
                  <button onClick={(e) => handleViewResume(e, app.resumeUrl)} className="resume-link-btn">
                    📄 Resume
                  </button>
                  ...
                  <div className="application-action-btns">
                    {app?.status === "interview_scheduled" ? (
                      ...
                      requiresAdvance(app) ? (
                        ...
                        <div className="hired-status-pill" style={{ color: "#4338ca", fontWeight: "bold" }}>
                          ✔ Selected (No Advance)
                        </div>
                      )
                    ) : app?.status === "hired" ? (
                      <div className="hired-status-pill" style={{ color: "#10b981", fontWeight: "bold" }}>
                        ✔ Hired & Paid
                      </div>
                    ) : (
                      app?.status !== "rejected" && (
                        <>
                          <button className="shortlist-btn" onClick={() => updateStatus(app?._id, "shortlisted")}>
                            Shortlist
                          </button>
                          <button className="reject-btn" onClick={() => updateStatus(app?._id, "rejected")}>
                            Reject
