...
//     } catch (err) {
//       console.error("Error posting project:", err.response || err);
//       // This will now catch the "Project limit reached" or "Validation" errors from your backend
//       alert(err.response?.data?.message || "Failed to post requirement.");
//     } finally {
//       setLoading(false);
//     }
...
const PostProjectForm = ({ companyId, onPost }) => {
  const [formData, setFormData] = useState({
    ...
    durationDays: "",
    perDayPayment: "",
    paymentTerms: "100% Post Completion", // Default value
    tfaProvided: false,
    tocProvided: false,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({
        ...
        durationDays: "",
        perDayPayment: "",
        paymentTerms: "100% Post Completion",
        tfaProvided: false,
        tocProvided: false,
      });
  ...
  return (
    <div className="form-wrapper glass">
      ...
      <form onSubmit={handleSubmit} className="project-form">
        ...
        {/* Row 4: Payment Terms (Updated to Select) */}
        <div className="form-group">
          <label>Payment Terms</label>
          <select
            name="paymentTerms"
            ...
            required
          >
            <option value="50% Advance & 50% Post Completion">50% Advance & 50% Post Completion</option>
            <option value="100% Post Completion">100% Post Completion</option>
            <option value="Within 15 days Post Completion">Within 15 days Post Completion</option>
          </select>
          <p style={{ fontSize: "0.75rem", color: "#666", marginTop: "5px" }}>
            * Choosing "Advance" will trigger a payment request upon trainer selection.
          </p>
        </div>
